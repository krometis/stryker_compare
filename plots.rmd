...
```{r cars}
get_data = function(filename,srcname,readcols=c("variable","q025","median","q975")) {
...
pd = position_dodge(0.5)
ggplot(data=etaStats, aes(x=Variant, y=median, color=Method)) +
...
#factors
#factors = c("eta.mean","eta.ratio","eta.std","beta.mean","beta.ratio","beta.std","dw.factor")
factors = c("eta.mean","eta.ratio","beta.mean","beta.ratio","dw.factor")
factorLabors = c("Scale Mean","Scale Std/Mean","Shape Mean","Shape Std/Mean","Downweight Factor")
```
...
factor_p = ggpubr::ggarrange(plotlist=factor_plots, nrow=3, common.legend = TRUE)
ggsave(paste0(outdir,"sensitivity.png"), factor_p, dpi = 300, width=8, height=3)
ggsave(paste0(outdir,"sensitivity.pdf"), factor_p, dpi = 300, width=8, height=3)
```
